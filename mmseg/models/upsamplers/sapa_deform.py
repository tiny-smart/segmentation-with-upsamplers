...
class SAPADeform(nn.Module):
    def __init__(self, dim_y, dim_x=None, scale_factor=2, num_point=9, groups=4, embedding_dim=32,
        ...
        if self.high_offset:
            offset_dim = groups * self.num_point * 2 * self.scale_factor ** 2
        else:
    ...
    def forward(self, y, x):
        ...
        if self.high_offset:
            offset = F.pixel_shuffle(self.offset(x_), upscale_factor=self.scale_factor)
        else:
            offset = F.interpolate(self.offset(x_), scale_factor=self.scale_factor)

        return self.attention(q, k, x, offset)

    def attention(self, q, k, x, offset):
        attn = F.softmax(deform_qk(q, k, offset, self.num_point, self.scale_factor, self.groups), dim=2)
        return deform_av(attn, x, offset, self.num_point, self.scale_factor, self.groups)
